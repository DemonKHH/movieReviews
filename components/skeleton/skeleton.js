...
Component({
  ...
  methods: {
    ...
    radiusHandle: function() {
      const that = this;
      // console.log(wx.createSelectorQuery().selectAll(`.skeleton >>>.skeleton-radius`))
    ...
    customHandle: function () {
      const that = this;
      wx.createSelectorQuery().selectAll(`.${this.data.selector} >>> .${this.data.selector}-custom`).boundingClientRect().exec(function (res) {
      });
    },
  }
